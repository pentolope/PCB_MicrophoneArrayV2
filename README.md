...
by [tools/patch_board.py](tools/patch_board.py). Test points are not populated.

Outstanding items are quality rather than function: 31 sub-0.05 mm segments and
69 corners sharper than 45°, left by the autorouter, are worth a cleanup pass.
The board is no longer regenerated by `tools/gen_pcb.py` — see
[docs/status.md](docs/status.md). No release has been sealed.
